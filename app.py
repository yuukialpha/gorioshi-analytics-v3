...
@app.route("/hourly/<key>/<email>/<zone_id>/<int:hours>")
def index_hourly(key, email, zone_id, hours):
...
@app.route("/daily/<key>/<email>/<zone_id>/<int:days>")
def index_daily(key, email, zone_id, days):
    transport = gql.transport.aiohttp.AIOHTTPTransport(url="https://api.cloudflare.com/client/v4/graphql", headers={
        "Authorization": f"Bearer {key}",
        "X-Auth-Email": email,
    })
    ...
    query_text = pathlib.Path("query2.txt").read_text("UTF-8")
    query = gql.gql(query_text)
    params = {
        "zoneTag": zone_id,
        "start": (datetime.datetime.now() - datetime.timedelta(days=days)).astimezone(datetime.timezone.utc).date().strftime('%Y-%m-%d'),
        "end": datetime.datetime.now().astimezone(datetime.timezone.utc).date().strftime('%Y-%m-%d'),
        "max": days,
    }

    result = client.execute(query, variable_values=params)
    first_group = result["viewer"]["zones"][0]["httpRequests1dGroups"]
    ...
    fig = matplotlib.figure.Figure()
    fig.suptitle(f"Stats for {zone_id} (daily)")
    fig.set_size_inches(7, 7)
    ax1, ax2 = fig.subplots(2)
    ax1.set_title("Users")
    ax1.plot(ax1_x, ax1_y)
